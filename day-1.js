...
const adventReducer = (frequenciesLog = {}, frequenciesInput = [], initialFrequency = 0) => {
	let result = null

	const reducer = (values) => values.reduce((currentFrequency, frequencyChange) => {
		const resultingFrequency  = currentFrequency + frequencyChange

		let storedFrequency = frequenciesLog[resultingFrequency]
		if (storedFrequency == null) {
			storedFrequency = frequenciesLog[resultingFrequency] = 1
		} else {
			storedFrequency = frequenciesLog[resultingFrequency] += 1

			result = result != null ? result : resultingFrequency
		}
	...
	return {
		accFrequency: reducer(frequenciesInput),
		frequenciesLog: frequenciesLog,
		frequencyRepetition: result
	}
}

// ---

const partOne = (frequenciesInput) => {
	const { accFrequency } = adventReducer({}, frequenciesInput, 0)

	console.log(` P1: Starting with a frequency of zero, what is the resulting frequency\n    after all of the changes in frequency have been applied?`)
	console.log(` A1: '${ accFrequency }'\n`)
}

const partTwo = (frequenciesInput) => {
	let frequenciesLog = {}
	let accFrequency = 0

	let frequencyRepetition = null
	while(true) {
		let result = adventReducer(frequenciesLog, frequenciesInput, accFrequency)

		frequencyRepetition = result.frequencyRepetition

		if (frequencyRepetition != null) break

		frequenciesLog = result.frequenciesLog
		accFrequency = result.accFrequency
	}

	console.log(` P2: What is the first frequency your device reaches twice?`)
	console.log(` A2: '${ frequencyRepetition }'`)
